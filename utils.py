...
def validate(model, dataloader, args):
    ...
    with torch.no_grad():
        for step, tokened_data_dict in enumerate(dataloader):
            ...
            tokened_data_dict.pop('offset_mapping')
            ret_data = model(**tokened_data_dict)
            tmp_eval_loss, logits = ret_data.loss, ret_data.logits
            ...
            _, out_classes_raw = torch.max(logits, -1)
    ...
    # 评估
    id2label = {v: k for k, v in args.label2id.items()}
...
def train(model, dataloader, optimizer, args):
    ...
    seq_len = dataloader.dataset.data['input_ids'].shape[-1]
    all_pred = torch.zeros(0, seq_len)
    all_labels = torch.zeros(0, seq_len)
    ...
    for batch_index, (tokened_data_dict) in enumerate(tqdm_batch_iterator):
        ...
        #     print(v)
        ret_data = model(**tokened_data_dict)
        loss, logits = ret_data.loss, ret_data.logits
    ...
    # correct = (all_pred == all_labels).sum()
    # correct = all_pred.equal(all_labels_)
    # accuracy = correct / all_pred.shape[0]
    # torch.equal()
    # accuracy = correct_predictions(all_pred, all_labels, args)
    # correct = (out_classes == targets).sum()
...
def my_plot(train_acc_list, dev_acc_list, losses, args):
    plt.figure()
    plt.plot(train_acc_list, color='r', label='train_f1')
    plt.plot(dev_acc_list, color='b', label='dev_f1')
    x = [i for i in range(len(train_acc_list))]
    for add, list_ in enumerate([train_acc_list, dev_acc_list]):
        ...
        plt.hlines(max_y, 0, max_x, colors='r' if add == 0 else 'b', linestyles='dashed')
    plt.legend()
    plt.savefig(os.path.join(os.path.dirname(args.train_file), f'{args.model}_f1.png'))
    plt.figure()
    plt.plot(losses)
