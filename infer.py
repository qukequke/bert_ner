...
def set_args():
    ...
    parser.add_argument('--pretrain_dir', default='./models/cner/bert/', type=str, required=False, help='预训练结果文件夹')
...
class Inferenve:
    ...
    def __init__(self):
        ...
        self.classifier = pipeline("ner", model=model, tokenizer=tokenizer, config=config, aggregation_strategy='simple')
        # self.classifier = pipeline("ner", model=model, tokenizer=tokenizer, config=config, aggregation_strategy=None)
