...
class DataPrecessForSentence(Dataset):
    ...
    def __init__(self, bert_tokenizer, LCQMC_file, args):
        """
        bert_tokenizer :分词器
        LCQMC_file     :语料文件
        """
        self.bert_tokenizer = bert_tokenizer
        self.data = self.get_input(LCQMC_file, args)
    ...
    def get_input(self, file, args):
        ...
        """
        if args.read_n_num:
            df = pd.read_csv(file, engine='python', encoding=args.csv_encoding, error_bad_lines=False, nrows=args.read_n_num)
        else:
            df = pd.read_csv(file, engine='python', encoding=args.csv_encoding, error_bad_lines=False)
        # print(df)
        self.length = len(df)
        self.bert_tokenizer.model_max_length = args.max_seq_len
        print(f"数据集个数为{len(df)}")
        df[args.text_col_name] = df[args.text_col_name].apply(lambda x: ' '.join(list(x)))  # 字之间加空格，处理英文分词错误
        ...
        data = self.bert_tokenizer(sentences, padding=True, truncation=True, return_tensors='pt', return_offsets_mapping=True)
        # 返回结果为类字典 {'input_ids':[[1,1,1,], [1,2,1]], 'token_type_ids':矩阵, 'attention_mask':矩阵,...}
        # from torch.utils.t
        seq_len = data['input_ids'].size(1)
        if args.class_col_name in df.columns:
            labels = df[args.class_col_name].tolist()
            self.labels = labels.copy()
            labels = [eval(i) for i in labels]
            labels = [(i + ([args.label2id['pad'], ] * (seq_len - len(i)))) if seq_len > len(i) else i[:seq_len]
